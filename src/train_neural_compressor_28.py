...
class Digit_Dataset(Dataset):
    def __init__(
        self,
        mode="train",
        ROOT_FD="/workspace/nttrang2108/VHT/Neural_Compression/",
        transforms=None,
    ):

        if mode == "train":
            self.data = MNIST(
                root=ROOT_FD, transform=transforms, train=True, download=True
            )
            self.data = torch.utils.data.Subset(self.data, range(1000))
        elif mode == "val":
            self.data = MNIST(
                root=ROOT_FD, transform=transforms, train=False, download=True
            )
...
class Training_Model:
    ...
    def eval(self, epochs, max_patience, entropy_coding_type):
        ...
        if total_loss < self.best_val_loss:
            ...
            torch.save(
                self.model,
                "../weights/img_28/img_28_indp/best_model_{}_{}_{}.model".format(
...
def get_input():
    ...
    parser.add_argument("--D", type=int, default=784)
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        training_model.eval(epoch, args.max_patience, args.entropy_coding_type)
