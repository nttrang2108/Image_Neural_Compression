...
class Encoder(nn.Module):
    def __init__(self, D, M, C):
        ...
        self.encode = nn.Sequential(nn.Linear(D, M*2),
                                    nn.BatchNorm1d(M*2),
                                    nn.ReLU(),
                                    nn.Linear(M*2, M),
                                    nn.BatchNorm1d(M),
...
class Decoder(nn.Module):
    def __init__(self, D, M, C):
        ...
        self.decode = nn.Sequential(nn.Linear(C, M//2),
                                    nn.BatchNorm1d(M//2),
                                    nn.ReLU(),
                                    nn.Linear(M//2, M),
                                    nn.BatchNorm1d(M),
...
class Casual_Conv1d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation, stride=1, A=False, **kwargs):
        ...
        self.conv1d = nn.Conv1d(
            in_channels, out_channels, kernel_size, padding=0, dilation=dilation, **kwargs)
...
class ARM_Entropy_Coding(nn.Module):
    def __init__(self, code_dim, codebook_dim, E=8, M_kernels=32, kernel=4):
        ...
        self.codebook_dim = codebook_dim
        self.arm_net = nn.Sequential(Casual_Conv1d(in_channels=1, out_channels=M_kernels, dilation=1, kernel_size=kernel, A=True, bias=True),
                                     nn.LeakyReLU(),
                                     Casual_Conv1d(in_channels=M_kernels, out_channels=M_kernels,
                                                   dilation=1, kernel_size=kernel, A=False, bias=True),
                                     nn.LeakyReLU(),
                                     Casual_Conv1d(in_channels=M_kernels, out_channels=E, dilation=1, kernel_size=kernel, A=False, bias=True))
...
class Neural_Compressor(nn.Module):
    ...
    def forward(self, x, reduction='avg'):
        ...
        Rate = torch.mean(self.entropy_coding(
            quantizer_out[0], quantizer_out[2]), 1)
...
class Training_Model:
    ...
    def train(self, epochs):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            pbar.set_description("Train_Loss: {:.4f} Train_Distortion: {:.4f} Train_Rate: {:.4f}".format(
        ...
        wandb.log({'train_loss': total_loss,
                  'train_distortion': total_distortion, 'train_rate': total_rate})
    ...
    def eval(self, epochs, max_patience):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            total_distortion += distortion.item()
            total_rate += rate.item()
            pbar.set_description("Val_Loss: {:.4f} Val_Distortion: {:.4f} Val_Rate: {:.4f}".format(
                loss, distortion, rate))

        total_loss = round(total_loss / len(self.val_loader.dataset), 4)
        total_distortion = round(
            total_distortion / len(self.val_loader.dataset), 4)
        ...
        wandb.log({'loss_val': total_loss,
                  'distortion_val': total_distortion, 'rate_val': total_rate})
...
if __name__ == "__main__":
    ...
    if args.entropy_coding_type == 'arm':
        kernel = 4
        entropy_coding = ARM_Entropy_Coding(
            code_dim=args.C, codebook_dim=args.E, E=args.E, M_kernels=args.M_kernels, kernel=kernel)

    model = Neural_Compressor(encoder=encoder, decoder=decoder,
                              entropy_coding=entropy_coding, quantizer=quantizer, beta=beta, detaching=False)

    optimizer = torch.optim.Adamax([p for p in model.parameters() if p.requires_grad], lr=args.lr)
    
    wandb.init(
        project="Image_Compressor",
        group="Neural_Compressor",
        name="neural_compressor_{}_{}_{}".format(args.entropy_coding_type, args.epochs, args.lr),
    )
    
    wandb.config.update(args)
    
    training_model = Training_Model(model=model, optimizer=optimizer, dict_dataloader=dict_dataloader)
